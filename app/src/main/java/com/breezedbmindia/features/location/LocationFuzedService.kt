package com.breezedbmindia.features.location
...
import com.breezedbmindia.app.utils.AppUtils.Companion.getDateTimeFromTimeStamp
import com.breezedbmindia.app.utils.AppUtils.Companion.isLocationActivityUpdating
import com.breezedbmindia.app.utils.FTStorageUtils
import com.breezedbmindia.app.utils.NotificationUtils
import com.breezedbmindia.base.BaseResponse
import com.breezedbmindia.base.presentation.BaseActivity
import com.breezedbmindia.base.presentation.BaseActivity.Companion.isMeetingUpdating
import com.breezedbmindia.base.presentation.BaseActivity.Companion.isShopActivityUpdating
import com.breezedbmindia.features.addshop.api.AddShopRepositoryProvider
import com.breezedbmindia.features.addshop.model.AddShopRequestCompetetorImg
...
import com.breezedbmindia.features.dashboard.presentation.SystemEventReceiver
...
import com.breezedbmindia.features.gpsstatus.GpsReceiver
import com.breezedbmindia.features.gpsstatus.LocationCallBack
import com.breezedbmindia.features.location.LocationWizard.Companion.NEARBY_RADIUS
import com.breezedbmindia.features.location.api.LocationRepoProvider
import com.breezedbmindia.features.location.ideallocapi.IdealLocationRepoProvider
...
import com.breezedbmindia.features.orderhistory.model.LocationUpdateRequest
...
//        if (AppUtils.isShopVisited) {
    ...
    private fun storeLmsUserInfo() {
        val sharedPreferences = getSharedPreferences("ModulePrefs", 0)
        var activity: LmsUserInfoEntity = LmsUserInfoEntity()
        activity = LmsUserInfoEntity()
        if (AppUtils.getLongTimeStampFromDate2(Pref.login_date!!) != AppUtils.getLongTimeStampFromDate2(AppUtils.getCurrentDateChanged())) {

            sharedPreferences.edit().clear().commit()

            AppDatabase.getDBInstance()?.lmsUserInfoDao()?.insertAll(activity.apply {
                if (sharedPreferences.getInt("DailyEntryCountOfLeaderboardLms", 0)
                        .toString() != null
                ) {
                    module_name = "LeaderboardLmsFrag"
                } else {
                    module_name = ""
                }
                count_of_use =
                    sharedPreferences.getInt("DailyEntryCountOfLeaderboardLms", 0).toString()
                time_spend =
                    sharedPreferences!!.getString("total_time_leaderboard_lms", "").toString()
                last_current_loc_lat = Pref.current_latitude
                last_current_loc_long = Pref.current_longitude
                last_current_loc_address = Pref.current_address
                date_time = sharedPreferences.getString("last_opening_date_leaderboard_lms", null)
                    .toString() + " " + sharedPreferences.getString(
                    "last_opening_time_leaderboard_lms",
                    null
                ).toString()
                phone_model = Build.BRAND.toUpperCase() + " : " + Build.MODEL
